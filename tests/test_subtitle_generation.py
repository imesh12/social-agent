...
from database.models import Audio, Script, Subtitle, Topic, TopicStatus, Video
...
def create_video() -> Video:
    ...
    try:
        ...
        audio = Audio(script=script, path="storage/audio/subtitle_test_audio.mp3")
        video = Video(audio=audio, path="storage/videos/subtitle_test_video.mp4")
        db.add(video)
        db.commit()
        db.refresh(video)
        return video
...
def test_generate_subtitles() -> None:
    app.dependency_overrides[get_manager_agent] = override_manager_agent
    try:
        with TestClient(app) as client:
            video = create_video()
            response = client.post("/generate-subtitles", json={"video_id": video.id})

        expected_path = f"storage/subtitles/subtitle_{video.id}.srt"
        assert response.status_code == 200
        assert response.json() == {"subtitle_path": expected_path}

        subtitle_file = Path(expected_path)
        ...
        db = SessionLocal()
        try:
            subtitle = db.query(Subtitle).filter(Subtitle.video_id == video.id).one()
            assert subtitle.path == expected_path
        finally:
            db.close()
    finally:
...
def test_generate_subtitles_returns_404_for_missing_video() -> None:
    app.dependency_overrides[get_manager_agent] = override_manager_agent
    try:
        with TestClient(app) as client:
            response = client.post("/generate-subtitles", json={"video_id": 999999})
